...
class ExternalWork < ActiveRecord::Base
  ...
  index_name "ao3_#{Rails.env}_external_works"
  document_type 'external_work'

  settings index: { number_of_shards: 5 } do
    mappings do
      indexes :url, analyzer: 'not_analyzed'
    end
  end
  ...
  def as_indexed_json(options={})
    as_json(
      root: false,
      only: [:id, :author, :notes, :summary, :title, :url, :created_at]
    )
  end
end
